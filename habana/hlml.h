...
#define HLML_EVENT_DRAM_DERR		(1 << 3)
/* Event about single bit ECC errors. */
#define HLML_EVENT_ECC_SERR		(1 << 4)
/* Memory location DRAM single error*/
#define HLML_EVENT_DRAM_SERR		(1 << 5)
...
typedef struct hlml_pci_cap {
	...
	char link_max_speed[PCI_LINK_INFO_LEN];
	char link_max_width[PCI_LINK_INFO_LEN];
} hlml_pci_cap_t;
 ...
 * pci_rev - The PCI revision, low byte of class word
 */
...
typedef enum hlml_err_counter_idx {
	...
	HLML_ECC_ERR_COUNTER_MAX
} hlml_err_counter_idx_t;
...
typedef struct hlml_aip_error_counters {
	unsigned long long err_counters[HLML_ECC_ERR_COUNTER_MAX];
	unsigned int index;
} hlml_aip_error_counters_t;
